...
# 运行方式一
nohup java -jar /app/software/devcaikun-1.0.0.jar -Xms200m -Xmx200m --spring.profiles.active=prod > /dev/null 2>&1 &

# 运行方式二
nohup java -jar /app/software/devcaikun-1.0.0.jar -Xms200m -Xmx200m --spring.profiles.active=prod > output.log 2>&1 &

# 运行方式三-代控制台打印
java -jar /app/software/devcaikun-1.0.0.jar --spring.profiles.active=prod

# 查看java运行情况
ps -ef | grep devcaikun
